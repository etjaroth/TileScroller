...
Compute_Shader::Compute_Shader(const char* shader_file, int x, int y, int z) {
	tex_x = x;
	tex_y = y;
	tex_z = z;
...
void Compute_Shader::dontuse() const {
	glBindVertexArray(0);
}

void Compute_Shader::fill_texture(GLuint tex) {
	use();
...
GLuint Compute_Shader::generate_texture(bool fill) {
	GLuint tex_output;
	...
	if (fill) {
		fill_texture(tex_output);
	}
	return tex_output;
}

void Compute_Shader::print_info() {
...
// ------------------------------------------------------------------------
void Compute_Shader::set_uint(const std::string& name, unsigned int value) const
{
...
void Compute_Shader::set_vec4(const std::string& name, float x, float y, float z, float w)
{
